...
void DSTAR::viterbi(byte* output, byte* history, byte* decoded)                       //viterbi decode
{
    ...
    for(n = 0; n < 2; n ++) for(m = 0; m < 4; m ++)
        {
            acc_error[n][m] = 0;    //clear acc_error_shift_table
        }
    ...
    for(m = 0; m < 4; m ++)
    {
        DSTAR::compute_error_branch(2, m, (output[0] >> 2) & 3, history);
    }
    for(m = 0; m < 4; m ++)
    {
        DSTAR::compute_error_branch(3, m, output[0] & 3, history);
    }
    ...
    for(n = (size_buffer * 8) - 1; n > 0; n --)
    {
        ...
        {
        case 0 :
            predecessor_state = (history[n] & 0x03);
            break;
        case 1 :
            predecessor_state = (history[n] & 0x0c) >> 2;
            break;
        case 2 :
            predecessor_state = (history[n] & 0x30) >> 4;
            break;
        case 3 :
            predecessor_state = (history[n] & 0xc0) >> 6;
            break;
        }
